...
public class LinkedList<E extends Comparable<E>> {
	...
	private int sizeHelper(Node ref) {
		if (ref == null) {
			return 0;
		} else {
	...
	public boolean exists(String elem) {
		return existsHelper(elem, first);
	}

	private boolean existsHelper(String elem, Node ref) {
		if (ref == null) {
			return false;
		} else if (ref.data.equals(elem)) {
			return true;
		} else {
			return existsHelper(elem, ref.next);
		}
	}
...
// Duplicate el next to each occurrence of el in this list.
	...
	private void duplicateAllHelper(E elem, Node ref) {
		...
		duplicateAllHelper(elem, ref.next);
